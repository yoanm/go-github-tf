...
func appendBranchResources(body *hclwrite.Body, c *GhRepoConfig, valGen tfsig.ValueGenerator, repoTfId string) {
	if c.Branches != nil {
		...
		for _, k := range keys {
			...
			sig := ghbranch.NewSignature(MapToBranchRes(k, v, valGen, c, repoTfId, LinkToRepository, LinkToBranch))
			if sig != nil {
				// In case
				...
				ignoreSourceBranch = ignoreSourceBranch || (hasDefaultBranch && v.SourceBranch != nil && *c.DefaultBranch.Name == *v.SourceBranch)
				...
				// Step 1 - add both current (if not already there) and new branch config(s) with empty configuration (brName: {})
				//        - run github-tf to re-generate terraform config
				// Step 3 - if current default branch had no config previously: run terraform import github_branch.${repo}-${oldBranch} ${repo}:${oldBranch}
				//        - run terraform apply to add the new branch
				// Step 4 - switch default branch name
				//        - If old default branch wasn't protected and you don't want to keep old default branch config (branches->BRANCH_NAME), jump to Step 5
				//        - run github-tf to re-generate terraform config
				//        - run terraform apply
				...
				if ignoreSourceBranch {
					sig.Lifecycle(tfsig.LifecycleConfig{IgnoreChanges: []string{"source_branch"}})
				}
...
func appendBranchProtectionResourceContent(body *hclwrite.Body, c *GhRepoConfig, valGen tfsig.ValueGenerator, repoTfId string) {
	tfsig.AppendNewLineAndBlockIfNotNil(
		body,
		ghbranchprotect.New(
			// /!\ use LinkToRepository, so underlying repository will have to be created before creating the branch protection
			// /!\ use LinkToBranch, to explicitly bind the protection to the branch, so if something change on default_branch resource,
		...
		for _, branchProtectionConfig := range *c.BranchProtections {
			tfsig.AppendNewLineAndBlockIfNotNil(
				...
				ghbranchprotect.New(
					// /!\ use LinkToRepository, so underlying repository will have to be created before creating the branch protection
					// /!\ do not use LinkToBranch, else branch protection will be created only after branch is created
					// (which is useless and can be done in parallel) and in many cases related branch config doesn't exist anyway
					// (else it's simpler to move the protection config under 'protection' attribute of the related branch)
