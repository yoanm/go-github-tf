...
func loadTemplate[T any](tplName string, loaderFn func(s string) *T, finderFn func(c *T) *[]string, tplType string, path ...string) (tplList []*T, err error) {
	if len(path) > TemplateMaxDepth {
		return nil, fmt.Errorf("more than %d levels of %s template detected for %s", TemplateMaxDepth, tplType, strings.Join(path, "->"))
	}
	var tpl *T
...
func loadTemplateList[T any](tplNameList *[]string, loaderFn func(s string) *T, finderFn func(c *T) *[]string, tplType string, path ...string) (tplList []*T, err error) {
	if tplNameList != nil {
